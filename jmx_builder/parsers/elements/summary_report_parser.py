...
from jmx_builder.models.tree import SummaryReport
from jmx_builder.parsers.const import *


class SummaryReportParser(TreeElementParser):
    @staticmethod
    def parse(xml_content: str) -> SummaryReport:
        testname = TreeElementParser.extract_attribute(xml_content, ATTR_TESTNAME) or "Summary Report"
        enabled_attr = TreeElementParser.extract_attribute(xml_content, ATTR_ENABLED)
        enabled = enabled_attr != "false"
        
        collector = SummaryReport(testname=testname, enabled=enabled)
